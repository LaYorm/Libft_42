...
/*   Updated: 2025/11/18 09:59:49 by yorimek          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_write_in_lign(char *tab, char const *s, char c, int *start_wrd)
{
	int	i;
...
static int	ft_malloc_lign(char const *s, char c, char	**tab, int nb_words)
{
	int	start_wrd;
	int	size_wrd;
	int	word;
	...
	while (s[start_wrd] && word < nb_words)
	{
		...
		ft_write_in_lign(tab[word], s, c, &start_wrd);
		word++;
	}
	tab[word] = NULL;
	return (1);
...
char	**ft_split(char const *s, char c)
{
	...
	if (!tab)
		return (NULL);
	if (!ft_malloc_lign(s, c, tab, nb_words))
		return (NULL);
	return (tab);
}
